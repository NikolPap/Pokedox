function getMainPokemonTemplate(i, bgColor) {
  return `<button style="background-color: ${bgColor};" class="open_dialog_button box_shadow" tabindex="1" onclick="openDialog(${i}, event, '${bgColor}')">
     ...
     <img class="main-image" src="${pokemons[i].image}" alt="${
    pokemons[i].name
  } image">
...
function getPokemonDialogTemplate(i, bgColor) {
  return `
     ...
     <img  class="main-image-dialog"   src= "${pokemons[i].image}" alt = "${pokemons[i].name} image">
...
function getDialogMainTemplate(i) {
  ...
  <table class="pokemon-stats">
    <tbody>
      <tr>
        <td class="td_left td_start">Height</td>
        <td class="td_start">:${pokemons[i].height}</td>
      </tr>
      <tr>
        <td class="td_left">Weight</td>
        <td>:${pokemons[i].weight}kg</td>
      </tr>
      <tr>
        <td class="td_left">Base Experience</td>
        <td>:${pokemons[i].base_experience}</td>
      </tr>
      <tr>
        <td class="td_left td_end">Abilities</td>
        <td class="td_end td_wrap">
